...
public class FavoritesAdapter extends RecyclerView.Adapter<FavoritesAdapter.MyHolder> {
    ...
    private ArrayList<Favorites> topicsArrayList;
    private Context context;
    private FirebaseUser firebaseUser;

    public FavoritesAdapter(ArrayList<Favorites>topicsArrayList,Context context){
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        ...
        final Favorites favorites = topicsArrayList.get(position);

        firebaseUser= FirebaseAuth.getInstance().getCurrentUser();

        //holder.recyclerRowBinding.topicText.setText(favorites.getFav());

        holder.itemView.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                if (favorites.getFavOwnerId().equals(firebaseUser.getUid())){
                            ...
                            .setPositiveButton("Remove", R.drawable.ic_baseline_delete_24, new AbstractDialog.OnClickListener() {
                                ...
                                public void onClick(dev.shreyaspatil.MaterialDialog.interfaces.DialogInterface dialogInterface, int which) {
                                    FirebaseDatabase.getInstance("https://friendb-76be3-default-rtdb.europe-west1.firebasedatabase.app/")
                                            .getReference().child("Favorites").child(firebaseUser.getUid()).child(favorites.getFavId()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                        @Override
